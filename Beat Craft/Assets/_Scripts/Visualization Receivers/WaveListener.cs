...
public class WaveListener : AudioEventListener {
	...
	public override void OnAudioEvent(AudioVisualizer.AudioEvent audioEvent) {
		...
		foreach (LineRenderer l in lineRenderers) {
			Vector2 mainTextureOffset = l.gameObject.GetComponent<Renderer>().material.mainTextureOffset;
			mainTextureOffset.x -= Time.deltaTime * 20 * mCur;
			l.gameObject.GetComponent<Renderer>().material.mainTextureOffset = mainTextureOffset;
		}
	}
}